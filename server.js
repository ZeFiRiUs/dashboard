...
function parseWoSheet(csv, sheetName) {
  ...
  const byPointMap = {};
  records.forEach(r => {
    if (!byPointMap[r.wh]) byPointMap[r.wh] = { total:0, byArticle:{}, byItem:{}, byArticleItem:{}, byItemArticleCost:{} };
    ...
    if (!byPointMap[r.wh].byItemArticleCost[r.item]) byPointMap[r.wh].byItemArticleCost[r.item] = {};
    byPointMap[r.wh].byItemArticleCost[r.item][r.article] =
      (byPointMap[r.wh].byItemArticleCost[r.item][r.article] || 0) + r.cost;
  });
  ...
  const byPoint = Object.entries(byPointMap)
    ...
    .map(([name, v]) => ({
      ...
      allItems: Object.entries(v.byItem)
        .sort((a, b) => b[1].cost - a[1].cost)
        .map(([item, d]) => {
          const arts = v.byItemArticleCost[item] || {};
          const article = Object.entries(arts).sort((a, b) => b[1] - a[1])[0]?.[0] || '';
          return { item, article, cost: Math.round(d.cost*100)/100, qty: Math.round(d.qty*1000)/1000, unit: d.unit };
        }),
